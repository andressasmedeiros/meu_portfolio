<template>
  <div id="sobre" class="pagina flex flex-column">
    <Toolbar class="toolbar-custom w-12" style="background-color: transparent; box-shadow: none; border: none;">
      <template #start>
        <span class="name-logo font-bold title reveal-text">
          andressa
        </span>
      </template>
      <template #end>
        <div class="menu-links flex gap-6">
          <a href="/CurriculoAndressa.pdf" download class="p-button-text no-underline flex items-center gap-2"
            style="color: #F05391">
            <i class="pi pi-download"></i>
            Currículo
          </a>
          <a href="#sobre" class="p-button-text no-underline">Sobre Mim</a>
          <a href="#habilidades" class="p-button-text no-underline">Habilidades</a>
          <a href="#projetos" class="p-button-text no-underline">Projetos</a>
          <a href="#contato" class="p-button-text no-underline">Contato</a>
        </div>
      </template>
    </Toolbar>

    <div class="lampada-fixed">
      <div class="lampada-wrapper">
        <svg class="toggle-scene" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 197.451 481.081">
          <defs>
            <marker id="e" orient="auto" overflow="visible" refx="0" refy="0">
              <path class="toggle-scene__cord-end" fill-rule="evenodd" stroke-width=".2666"
                d="M.98 0a1 1 0 11-2 0 1 1 0 012 0z"></path>
            </marker>
            <marker id="d" orient="auto" overflow="visible" refx="0" refy="0">
              <path class="toggle-scene__cord-end" fill-rule="evenodd" stroke-width=".2666"
                d="M.98 0a1 1 0 11-2 0 1 1 0 012 0z"></path>
            </marker>
            <marker id="c" orient="auto" overflow="visible" refx="0" refy="0">
              <path class="toggle-scene__cord-end" fill-rule="evenodd" stroke-width=".2666"
                d="M.98 0a1 1 0 11-2 0 1 1 0 012 0z"></path>
            </marker>
            <marker id="b" orient="auto" overflow="visible" refx="0" refy="0">
              <path class="toggle-scene__cord-end" fill-rule="evenodd" stroke-width=".2666"
                d="M.98 0a1 1 0 11-2 0 1 1 0 012 0z"></path>
            </marker>
            <marker id="a" orient="auto" overflow="visible" refx="0" refy="0">
              <path class="toggle-scene__cord-end" fill-rule="evenodd" stroke-width=".2666"
                d="M.98 0a1 1 0 11-2 0 1 1 0 012 0z"></path>
            </marker>
            <clippath id="g" clippathunits="userSpaceOnUse">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="4.677"
                d="M-774.546 827.629s12.917-13.473 29.203-13.412c16.53.062 29.203 13.412 29.203 13.412v53.6s-8.825 16-29.203 16c-21.674 0-29.203-16-29.203-16z">
              </path>
            </clippath>
            <clippath id="f" clippathunits="userSpaceOnUse">
              <path
                d="M-868.418 945.051c-4.188 73.011 78.255 53.244 150.216 52.941 82.387-.346 98.921-19.444 98.921-47.058 0-27.615-4.788-42.55-73.823-42.55-69.036 0-171.436-30.937-175.314 36.667z">
              </path>
            </clippath>
          </defs>
          <g class="toggle-scene__cords">
            <path class="toggle-scene__cord" marker-end="url(#a)" fill="none" stroke-linecap="square" stroke-width="6"
              d="M123.228-28.56v150.493" transform="translate(-24.503 256.106)"></path>
            <path class="toggle-scene__cord" marker-end="url(#a)" fill="none" stroke-linecap="square" stroke-width="6"
              d="M123.228-28.59s28 8.131 28 19.506-18.667 13.005-28 19.507c-9.333 6.502-28 8.131-28 19.506s28 19.507 28 19.507"
              transform="translate(-24.503 256.106)"></path>
            <path class="toggle-scene__cord" marker-end="url(#a)" fill="none" stroke-linecap="square" stroke-width="6"
              d="M123.228-28.575s-20 16.871-20 28.468c0 11.597 13.333 18.978 20 28.468 6.667 9.489 20 16.87 20 28.467 0 11.597-20 28.468-20 28.468"
              transform="translate(-24.503 256.106)"></path>
            <path class="toggle-scene__cord" marker-end="url(#a)" fill="none" stroke-linecap="square" stroke-width="6"
              d="M123.228-28.569s16 20.623 16 32.782c0 12.16-10.667 21.855-16 32.782-5.333 10.928-16 20.623-16 32.782 0 12.16 16 32.782 16 32.782"
              transform="translate(-24.503 256.106)"></path>
            <path class="toggle-scene__cord" marker-end="url(#a)" fill="none" stroke-linecap="square" stroke-width="6"
              d="M123.228-28.563s-10 24.647-10 37.623c0 12.977 6.667 25.082 10 37.623 3.333 12.541 10 24.647 10 37.623 0 12.977-10 37.623-10 37.623"
              transform="translate(-24.503 256.106)"></path>
            <g class="line toggle-scene__dummy-cord">
              <line marker-end="url(#a)" x1="98.7255" x2="98.7255" y1="240.5405" y2="380.5405"></line>
            </g>
            <circle class="toggle-scene__hit-spot" cx="98.7255" cy="380.5405" r="60" fill="transparent"></circle>
          </g>
          <g class="toggle-scene__bulb bulb" transform="translate(844.069 -645.213)">
            <path class="bulb__cap" stroke-linecap="round" stroke-linejoin="round" stroke-width="4.677"
              d="M-774.546 827.629s12.917-13.473 29.203-13.412c16.53.062 29.203 13.412 29.203 13.412v53.6s-8.825 16-29.203 16c-21.674 0-29.203-16-29.203-16z">
            </path>
            <path class="bulb__cap-shine" d="M-778.379 802.873h25.512v118.409h-25.512z" clip-path="url(#g)"
              transform="matrix(.52452 0 0 .90177 -368.282 82.976)"></path>
            <path class="bulb__cap" stroke-linecap="round" stroke-linejoin="round" stroke-width="4"
              d="M-774.546 827.629s12.917-13.473 29.203-13.412c16.53.062 29.203 13.412 29.203 13.412v0s-8.439 10.115-28.817 10.115c-21.673 0-29.59-10.115-29.59-10.115z">
            </path>
            <path class="bulb__cap-outline" fill="none" stroke-linecap="round" stroke-linejoin="round"
              stroke-width="4.677"
              d="M-774.546 827.629s12.917-13.473 29.203-13.412c16.53.062 29.203 13.412 29.203 13.412v53.6s-8.825 16-29.203 16c-21.674 0-29.203-16-29.203-16z">
            </path>
            <g class="bulb__filament" fill="none" stroke-linecap="round" stroke-width="5">
              <path d="M-752.914 823.875l-8.858-33.06"></path>
              <path d="M-737.772 823.875l8.858-33.06"></path>
            </g>
            <path class="bulb__bulb" stroke-linecap="round" stroke-width="5"
              d="M-783.192 803.855c5.251 8.815 5.295 21.32 13.272 27.774 12.299 8.045 36.46 8.115 49.127 0 7.976-6.454 8.022-18.96 13.273-27.774 3.992-6.7 14.408-19.811 14.408-19.811 8.276-11.539 12.769-24.594 12.769-38.699 0-35.898-29.102-65-65-65-35.899 0-65 29.102-65 65 0 13.667 4.217 26.348 12.405 38.2 0 0 10.754 13.61 14.746 20.31z">
            </path>
            <circle class="bulb__flash" cx="-745.343" cy="743.939" r="83.725" fill="none" stroke-dasharray="10,30"
              stroke-linecap="round" stroke-linejoin="round" stroke-width="10"></circle>
            <path class="bulb__shine" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="12"
              d="M-789.19 757.501a45.897 45.897 0 013.915-36.189 45.897 45.897 0 0129.031-21.957"></path>
          </g>
        </svg>
      </div>
    </div>

    <div class="container-imagem">
      <div class="w-full md:w-6 flex justify-content-center align-items-center">
        <img src="@/assets/logoPC.png" alt="Logo" class="logo-bounce imagem-logo">
      </div>
      <div class="w-12 md:w-6 flex justify-content-center align-items-center">
        <div class="text-white px-4 responsivo">
          <div class="flex flex-column font-geral">
            <h1 class="title_principal font-bold text-white mb-2 typewriter">
              <span class="texto-intro">Olá, aqui é a</span>
              <span class="font-semibold text-intro-name"> Andressa Medeiros</span>
            </h1>
            <p class="subtitle font-semibold font-geral mb-2">
              Desenvolvedora Full Stack
            </p>
            <p class="texto-responsivo mb-2">
              Gosto de criar soluções que realmente funcionam e fazem sentido para quem
              usa. Trabalho tanto no front-end quanto no back-end. Já
              colaborei em projetos de outros desenvolvedores, contribuindo com organização,
              integrações e melhorias no código.
            </p>
          </div>
        </div>
      </div>
    </div>
    <div class="w-12 fixed-bottom flex align-items-center font-geral" style="font-size: 11px;">
      <div class="flex flex-column" @click="goTo('sobre')">
        <span class="font-bold">Sobre</span>
      </div>
      <div class="flex flex-column" @click="goTo('habilidades')">
        <span class="font-bold">Habilidades</span>
      </div>
      <div class="flex flex-column" @click="goTo('projetos')">
        <span class="font-bold">Projetos</span>
      </div>
      <div class="flex flex-column" @click="goTo('contato')">
        <span class="font-bold">Contato</span>
      </div>
      <div class="flex flex-column">
        <a href="/CurriculoAndressa.pdf" download class="font-bold no-underline" style="color: #F05391;">
          Currículo
        </a>
      </div>
    </div>
  </div>
</template>

<script lang="js">
export default {
  methods: {
    goTo(id) {
      const el = document.getElementById(id)
      if (el) {
        el.scrollIntoView({ behavior: 'smooth' })
      }
    }
  },
  mounted() {
    const {
      gsap: { registerPlugin, set, to, timeline },
      MorphSVGPlugin,
      Draggable
    } = window;

    registerPlugin(MorphSVGPlugin);

    let startX, startY;
    const AUDIO = {
      CLICK: new Audio('https://assets.codepen.io/605876/click.mp3')
    };

    const STATE = { ON: false };
    const CORD_DURATION = 0.1;
    const CORDS = document.querySelectorAll('.toggle-scene__cord');
    const HIT = document.querySelector('.toggle-scene__hit-spot');
    const DUMMY = document.querySelector('.toggle-scene__dummy-cord');
    const DUMMY_CORD = DUMMY.querySelector('line');
    const PROXY = document.createElement('div');
    const ENDX = DUMMY_CORD.getAttribute('x2');
    const ENDY = DUMMY_CORD.getAttribute('y2');

    const RESET = () => {
      set(PROXY, { x: ENDX, y: ENDY });
    };
    RESET();

    const CORD_TL = timeline({
      paused: true,
      onStart: () => {
        STATE.ON = !STATE.ON;
        set(document.documentElement, { '--on': STATE.ON ? 1 : 0 });
        set([DUMMY, HIT], { display: 'none' });
        set(CORDS[0], { display: 'block' });
        AUDIO.CLICK.play();

        document.documentElement.classList.toggle('modo-claro', STATE.ON);
        document.body.classList.toggle('modo-claro', STATE.ON);
      },
      onComplete: () => {
        set([DUMMY, HIT], { display: 'block' });
        set(CORDS[0], { display: 'none' });
        RESET();
      }
    });

    for (let i = 1; i < CORDS.length; i++) {
      CORD_TL.add(to(CORDS[0], {
        morphSVG: CORDS[i],
        duration: CORD_DURATION,
        repeat: 1,
        yoyo: true
      }));
    }

    Draggable.create(PROXY, {
      trigger: HIT,
      type: 'x,y',
      onPress: e => {
        startX = e.x;
        startY = e.y;
      },
      onDrag() {
        set(DUMMY_CORD, { attr: { x2: this.x, y2: this.y } });
      },
      onRelease(e) {
        const DISTX = Math.abs(e.x - startX);
        const DISTY = Math.abs(e.y - startY);
        const TRAVELLED = Math.sqrt(DISTX ** 2 + DISTY ** 2);
        to(DUMMY_CORD, {
          attr: { x2: ENDX, y2: ENDY },
          duration: CORD_DURATION,
          onComplete: () => {
            if (TRAVELLED > 50) {
              CORD_TL.restart();
            } else {
              RESET();
            }
          }
        });
      }
    });
  }
}
</script>


<style scoped>
.pagina {
  position: relative;
  min-height: auto;
  background-color: #211d20;
}

.reveal-text {
  display: inline-block;
  overflow: hidden;
  white-space: nowrap;
  width: 0;
  animation: reveal 2s forwards cubic-bezier(0, 0, 0, 0.05);
}

.toolbar-custom a:visited {
  color: #ffffff;
}

.toolbar-custom a {
  font-family: 'Open Sans', sans-serif;
  background: none;
  padding: 0;
  font-size: 2rem;
  margin: 0;
  border: none;
  box-shadow: none;
  color: #ffffff;
  transition: transform 0.2s ease, text-shadow 0.2s ease;
}

.toolbar-custom i {
  font-size: 15px;
}

.fixed-bottom>div {
  padding: 0 0.5rem;
  transition: transform 0.2s ease, text-shadow 0.2s ease;
  cursor: pointer;
}

.fixed-bottom>div:hover {
  transform: translateY(-4px);
  text-shadow: 0 4px 12px rgba(157, 121, 162, 0.5);
}

.toolbar-custom a:hover {
  transform: translateY(-4px);
  text-shadow: 0 4px 12px rgba(157, 121, 162, 0.5);
}

.menu-links {
  display: none;
}

.fixed-bottom {
  position: fixed;
  bottom: 0;
  color: #ffffff;
  background-color: #211d20;
  z-index: 1000;
  height: 60px;
  justify-content: space-around;
  border-top: 1px solid white;
}

.logo-animated {
  position: absolute;
  top: 40%;
  left: 30%;
  animation: bounceY 0.6s ease-in-out 3;
}

.logo-bounce {
  animation: bounceFall 1.2s ease-out 1;
}

.toolbar-custom {
  justify-content: space-around !important;
}

.typewriter {
  overflow: hidden;
  border-right: 2px solid #fff;
  white-space: nowrap;
  animation: typing 3s steps(35, end), blink-caret 0.75s step-end infinite;
  width: fit-content;
  max-width: 100%;
}

.texto-responsivo {
  font-size: 12px;
  color: white;
  text-align: justify;
}

.title_principal {
  font-size: 16px;
}

.subtitle {
  font-size: 14px;
  color: #FFE9B6;
}

.responsivo {
  max-width: 800px;
  margin-left: -100px;
  transform: translateY(-25px);
}

.container-imagem {
  display: flex;
  flex-grow: 1;
  align-items: center;
  gap: 4rem;
}

.imagem-logo {
  width: 150px;
}

.name-logo {
  font-size: 8rem;
  color: #9D79A2;
}

body.modo-claro .name-logo {
  color: #52B7A9;
}

body.modo-claro .pagina {
  background-color: #ffffff;
}

body.modo-claro .fixed-bottom {
  background-color: #ffffff;
  color: #211d20;
  border-top: 1px solid #211d20;
}

body.modo-claro .texto-responsivo {
  color: #211d20;
}

.texto-intro {
  color: #ffffff;
}

body.modo-claro .texto-intro {
  color: #211d20;
}

body.modo-claro .subtitle {
  color: #9D79A2;
}

body.modo-claro .toolbar-custom a {
  color: #211d20;
}

body.modo-claro .text-intro-name {
  color: #F05391;
}

.text-intro-name {
  color: #52B7A9;
}

@media (max-width: 375px) {
  .responsivo {
    max-width: 100% !important;
  }
}

@media (max-width: 767px) {
  .title_principal {
    font-size: 22px;
  }

  .subtitle {
    font-size: 12px;
  }

  .responsivo {
    margin-left: 0;
  }

  .container-imagem {
    flex-direction: column;
    gap: 4rem;
    margin-top: 5rem;
  }

  .texto-responsivo {
    font-size: 16px;
    line-height: 1.5;
  }

  .imagem-logo {
    width: 220px;
  }

  .menu-links {
    display: none;
  }

  .name-logo {
    font-size: 4rem;
    margin: 0 auto;
    text-align: center;
    display: block;
  }

  .menu-links {
    display: none !important;
  }

  .toolbar-custom {
    justify-content: center !important;
  }
}

@media (min-width: 768px) {
  .texto-responsivo {
    font-size: 20px;
  }

  .title_principal {
    font-size: 33px;
  }

  .subtitle {
    font-size: 15px;
  }

  .responsivo {
    max-width: 400px;
    margin-left: -100px;
    transform: translateY(-25px);
  }

  .pagina {
    min-height: 100vh;
  }

  .imagem-logo {
    width: 250px;
  }

  .name-logo {
    font-size: 60px;
    margin-left: 41px;
  }

  .toolbar-custom a {
    font-size: 12px;
  }

  .menu-links {
    display: flex;
  }

  .fixed-bottom {
    display: none !important;
  }
}

@media (min-width: 1024px) {
  .texto-responsivo {
    font-size: 21px;
  }

  .title_principal {
    font-size: 28px;
  }

  .subtitle {
    font-size: 18px;
  }

  .responsivo {
    max-width: 500px;
    margin-left: -100px;
    transform: translateY(-25px);
  }

  .imagem-logo {
    width: 350px;
  }

  .toolbar-custom a {
    font-size: 18px;
  }

  .toolbar-custom i {
    font-size: 18px;
  }
}

@media (min-width: 1920px) {
  .texto-responsivo {
    font-size: 32px;
  }

  .title_principal {
    font-size: 48px;
  }

  .subtitle {
    font-size: 25px;
  }

  .responsivo {
    max-width: 800px;
    margin-left: -200px;
    transform: translateY(-25px);
  }

  .imagem-logo {
    width: 500px;
  }

  .name-logo {
    font-size: 6rem;
  }

  .toolbar-custom a {
    font-size: 20px;
  }

  .toolbar-custom i {
    font-size: 20px;
  }
}

@media (min-width: 2560px) {
  .texto-responsivo {
    font-size: 40px;
  }

  .title_principal {
    font-size: 54px;
  }

  .subtitle {
    font-size: 30px;
  }

  .responsivo {
    max-width: 900px;
    margin-left: -200px;
    transform: translateY(-25px);
  }

  .imagem-logo {
    width: 700px;
  }

  .name-logo {
    font-size: 8rem;
  }

  .toolbar-custom a {
    font-size: 35px;
  }

  .toolbar-custom i {
    font-size: 35px;
  }
}


@keyframes typing {
  from {
    width: 0;
  }

  to {
    width: 100%;
  }
}

@keyframes blink-caret {

  from,
  to {
    border-color: transparent;
  }

  50% {
    border-color: #fff;
  }
}

@keyframes bounceFall {
  0% {
    transform: translateY(-200px);
  }

  30% {
    transform: translateY(0);
  }

  50% {
    transform: translateY(-40px);
  }

  65% {
    transform: translateY(0);
  }

  75% {
    transform: translateY(-20px);
  }

  85% {
    transform: translateY(0);
  }

  92% {
    transform: translateY(-10px);
  }

  100% {
    transform: translateY(0);
  }
}

@keyframes reveal {
  from {
    width: 0;
  }

  to {
    width: 100%;
  }
}
</style>
